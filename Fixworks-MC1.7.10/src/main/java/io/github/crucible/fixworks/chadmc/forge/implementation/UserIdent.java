...
public class UserIdent {
    ...
    public @Nonnull String getPlayerName() {
        if (this.uuid != null) {
            String lastKnownName = UsernameCache.getLastKnownUsername(this.uuid);
    ...
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = prime * result + this.playerName.hashCode();
    ...
    public void saveToNbt(NBTTagCompound nbt, String prefix) {
        if (this.uuid != null) {
            nbt.setString(prefix + ".uuid", this.uuid.toString());
        }
        nbt.setString(prefix + ".login", this.playerName);
    }
