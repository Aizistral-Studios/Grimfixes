...
public abstract class MixinEntityDoppleganger extends EntityLiving implements IBotaniaBoss, IEntityAdditionalSpawnData {
    ...
    private void clearPotions(EntityPlayer player) {
        ...
        List<Potion> potionsToRemove = player.getActivePotionEffects().stream().filter(effect -> effect != null && effect.getPotion() != null && effect.getDuration() < 160 && effect.getIsAmbient() && !effect.getPotion().isBadEffect()).map(PotionEffect::getPotion).distinct().collect(Collectors.toList());

        for (Potion potion : potionsToRemove) {
            player.removePotionEffect(potion);
            ((WorldServer) this.world).getPlayerChunkMap().getEntry(posXInt >> 4, posZInt >> 4).sendPacket(new SPacketRemoveEntityEffect(player.getEntityId(), potion));
        }
    }
}