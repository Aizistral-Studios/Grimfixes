...
public abstract class MixinItemTFOreMagnet {
    ...
    @Shadow
    protected abstract Vec3d getOffsetLook(EntityLivingBase living, float yawOffset, float pitchOffset);
    ...
    private int doMagnet(World world, EntityLivingBase living, float yawOffset, float pitchOffset) {
        ...
        if (!EHIntegration.canBreak(playerMP, srcPos) || !EHIntegration.canBreak(playerMP, destPos))
            return 1; //Needs to return 1 to stop the process!
