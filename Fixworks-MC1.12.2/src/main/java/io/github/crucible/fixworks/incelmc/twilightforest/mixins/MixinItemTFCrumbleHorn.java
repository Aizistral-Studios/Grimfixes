...
public abstract class MixinItemTFCrumbleHorn {
    ...
    @Shadow @Final
    private List<Pair<Predicate<IBlockState>, UnaryOperator<IBlockState>>> crumbleTransforms;

    @Shadow @Final
    private static int CHANCE_CRUMBLE;

    @Shadow @Final
    private List<Predicate<IBlockState>> harvestedStates;

    @Shadow @Final
    private static int CHANCE_HARVEST;

    @Shadow
    protected abstract void postTrigger(EntityLivingBase living, ItemStack stack, World world, BlockPos pos);

    @Shadow
    protected abstract int crumbleBlocksInAABB(ItemStack stack, World world, EntityLivingBase living, AxisAlignedBB box);
    ...
    private int doCrumble(ItemStack stack, World world, EntityLivingBase living) {
        ...
        if (!EHIntegration.canBreak(playerMP, playerMP.getPosition()))
    ...
    private boolean crumbleBlock(ItemStack stack, World world, EntityLivingBase living, BlockPos pos) {
        ...
        if (!EHIntegration.canBreak((EntityPlayer) living, pos))
            return false;

        for (Pair<Predicate<IBlockState>, UnaryOperator<IBlockState>> transform : this.crumbleTransforms) {
        ...
        for (Predicate<IBlockState> predicate : this.harvestedStates) {
