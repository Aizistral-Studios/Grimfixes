...
public abstract class MixinItemMissileRod extends Item {
    ...
    @Shadow
    public abstract boolean spawnMissile(World world, EntityLivingBase thrower, double x, double y, double z);
    ...
    public void onUsingTick(ItemStack stack, EntityLivingBase living, int count) {
        ...
        if (!EHIntegration.canBreak(player, player.getPosition()))
            return;

        if (count != this.getMaxItemUseDuration(stack) && count % (IManaProficiencyArmor.Helper.hasProficiency(player, stack) ? 1 : 2) == 0 && !player.world.isRemote && ManaItemHandler.requestManaExactForTool(stack, player, 120, false)) {
            if (this.spawnMissile(player.world, player, player.posX + (Math.random() - 0.5 * 0.1), player.posY + 2.4 + (Math.random() - 0.5 * 0.1), player.posZ + (Math.random() - 0.5 * 0.1))) {
